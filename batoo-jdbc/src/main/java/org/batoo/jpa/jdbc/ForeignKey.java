...
public class ForeignKey {
		...
		if (mapping instanceof SingularAssociationMapping) {
			...
			if (mapsId != null) {
				...
				if (idClass != this.masterMapping.getJavaType()) {
					throw new MappingException("MapsId mapped attribute type " + this.masterMapping.getJavaType().getName()
						+ " is not compatible with target entity primary key type " + idClass.getName(), mapping.getLocator());
				}
			}
		}
	...
	private void linkJoinColumns(AssociationMapping<?, ?, ?> mapping, final SingularMapping<?, ?> idMapping) {
		final boolean id = (mapping instanceof SingularMapping) && ((SingularMapping<?, ?>) mapping).isId();
	...
	private void linkJoinColumnsWithMetadata(AssociationMapping<?, ?, ?> mapping, final SingularMapping<?, ?> idMapping) {
		...
		final boolean id = !virtual && (mapping instanceof SingularAssociationMapping) && ((SingularMapping<?, ?>) mapping).isId();
	...
	private AbstractColumn locateMasterColumn(AssociationMapping<?, ?, ?> mapping, final JoinColumn joinColumn) {
		final EntityTypeDescriptor type = (EntityTypeDescriptor) mapping.getRoot().getTypeDescriptor();
		...
		throw new MappingException("Primary key column cannot be located: " + joinColumn.getName() + ". Possible columns are: "
			+ type.getPrimaryTable().getPkColumnNames(), mapping.getLocator());
	}

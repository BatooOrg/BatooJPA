...
public final class DeploymentUnitTask implements Callable<Void>, Comparable<DeploymentUnitTask> {
	...
	private final boolean waitForMappedSuperClass;
	 ...
	 * @param waitForMappedSuperClass
	 *            wait for mapped super classes only if applicable
	 * 
	 * @since $version
	 * @author hceylan
	 */
	public DeploymentUnitTask(@SuppressWarnings("rawtypes") DeploymentManager manager, ManagedTypeImpl<?> type,
		boolean waitForMappedSuperClass) {
		super();

		this.manager = manager;
		this.type = type;
		this.waitForMappedSuperClass = waitForMappedSuperClass;
	}
	...
	public final Void call() throws Exception {
		if (this.type instanceof IdentifiableTypeImpl) {
			final IdentifiableTypeImpl<?> superType = ((IdentifiableTypeImpl<?>) this.type).getSupertype();

			// if the super type hasn't performed yet, then wait for to finish
			if ((superType != null) && (!(superType instanceof MappedSuperclassTypeImpl) || this.waitForMappedSuperClass)) {

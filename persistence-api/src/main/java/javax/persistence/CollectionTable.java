 ...
 * the embeddable class or from the basic type according to the default values of the <code>Column</code> annotation. In the case of a basic
 * type, the column name is derived from the name of the collection-valued field or property. In the case of an embeddable class, the column
 * names are derived from the field or property names of the embeddable class.
 * <ul>
 * <li>To override the default properties of the column used for a basic type, the <code>Column</code> annotation is used on the
 * collection-valued attribute in addition to the <code>ElementCollection</code> annotation.
 * 
 * <li>To override these defaults for an embeddable class, the <code>AttributeOverride</code> and/or <code>AttributeOverrides</code>
 * annotations can be used in addition to the <code>ElementCollection</code> annotation. If the embeddable class contains references to
 * other entities, the default values for the columns corresponding to those references may be overridden by means of the
 * <code>AssociationOverride</code> and/or <code>AssociationOverrides</code> annotations.
 * </ul>
 * 
 * <p>
 * If the <code>CollectionTable</code> annotation is missing, the default values of the <code>CollectionTable</code> annotation elements
 * apply.
 * 
 * <pre>
 *    Example:

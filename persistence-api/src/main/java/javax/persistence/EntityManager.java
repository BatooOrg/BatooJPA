 ...
 * 
 * <p>
 * An <code>EntityManager</code> instance is associated with a persistence context. A persistence context is a set of entity instances in
 * which for any persistent entity identity there is a unique entity instance. Within the persistence context, the entity instances and
 * their lifecycle are managed. The <code>EntityManager</code> API is used to create and remove persistent entity instances, to find
 * entities by their primary key, and to query over entities.
 * 
 * <p>
 * The set of entities that can be managed by a given <code>EntityManager</code> instance is defined by a persistence unit. A persistence
 * unit defines the set of all classes that are related or grouped by the application, and which must be colocated in their mapping to a
 * single database.
 * 
 * @see Query
 * @see TypedQuery
...
public interface EntityManager {
	 ...
	 *             be thrown when the persist operation is invoked, or the <code>EntityExistsException</code> or another
	 *             <code>PersistenceException</code> may be
	 ...
	 * <p>
	 * If the entity is found within the persistence context and the lock mode type is pessimistic and the entity has a version attribute,
	 * the persistence provider must perform optimistic version checks when obtaining the database lock. If these checks fail, the
	 ...
	 * <ul>
	 * <li>the <code>PessimisticLockException</code> will be thrown if the database locking failure causes transaction-level rollback
	 * <li>the <code>LockTimeoutException</code> will be thrown if the database locking failure causes only statement-level rollback
	 * </ul>
	 * 
	 * @param entityClass
	 *            entity class
	 ...
	 * permitted to throw the <code>EntityNotFoundException</code> when <code>getReference</code> is called.)
	 ...
	 * optimistic version checks when obtaining the database lock. If these checks fail, the <code>OptimisticLockException</code> will be
	 * thrown.
	 * <p>
	 * If the lock mode type is pessimistic and the entity instance is found but cannot be locked:
	 * <ul>
	 ...
	 * @throws TransactionRequiredException
	 *             if there is no
	 *             transaction and if invoked on a container-managed <code>EntityManager</code> instance with
	 ...
	 * provider-specific API. If the provider's <code>EntityManager</code> implementation does not support the specified class, the
	 ...
	 * on the <code>EntityManager</code> instance and any <code>Query</code> and <code>TypedQuery</code> objects obtained from it will throw
	 * the <code>IllegalStateException</code> except for <code>getProperties</code>, <code>getTransaction</code>, and <code>isOpen</code>

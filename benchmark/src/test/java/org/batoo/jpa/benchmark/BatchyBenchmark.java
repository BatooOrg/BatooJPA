...
public class BatchyBenchmark {
	...
	public static void boot() {
		...
		if (BatchyBenchmark.SUMMARIZE) {
			System.out.println("Batchy Benchmark==============================================");
	...
	protected void _measure(final long id) {
		...
		pool.submit(new Runnable() {
			...
			@Override
			public void run() {
				while (BatchyBenchmark.this.running) {
					BatchyBenchmark.this._measureSingleTime(id, mxBean);
					try {
						Thread.sleep(0, 10);
					}
	...
	public void _measureBefore() {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				BatchyBenchmark.this.running = true;
				BatchyBenchmark.this._measure(id);
			}
		}).start();
	...
	private void _measureSingleTime(long id, final ThreadMXBean mxBean) {
		...
		if (this.oldTime == 0) {
			...
			for (int i = threadInfo.getStackTrace().length - 1; i >= 0; i--) {
				...
				final String key = BatchyBenchmark.SUMMARIZE ? //
	...
	private void doBenchmarkRemove(final EntityManager em, final Person[] persons) {
		final EntityTransaction tx = em.getTransaction();
		tx.begin();
	...
	private void doBenchmarkUpdate(final EntityManager em, final Person[] persons) {
		final EntityTransaction tx = em.getTransaction();
		tx.begin();

		for (int i = 0; i < persons.length; i++) {
			persons[i].setName("Ceylan" + i);
		}
	...
	private void doRemove(final EntityManagerFactory emf, final List<Person>[] persons) {
		for (final List<Person> person : persons) {
			final EntityManager em = emf.createEntityManager();

			final Person[] persons2 = new Person[persons[0].size()];

			for (int j = 0; j < person.size(); j++) {
				persons2[j] = em.find(Person.class, person.get(j).getId());
			}

			this.doBenchmarkRemove(em, persons2);
		}
	}
	...
	private void doUpdate(final EntityManagerFactory emf, final List<Person>[] persons) {
		for (final List<Person> person : persons) {
			final EntityManager em = emf.createEntityManager();
			...
			this.doBenchmarkUpdate(em, persons2);
		}
	}
	...
	private void test(Type type, final EntityManagerFactory emf) {
		...
		for (int i = 0; i < BatchyBenchmark.BENCHMARK_LENGTH; i++) {
